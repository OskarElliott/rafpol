...
export default function AdminDashboard() {
  ...
  if (!data) return (
    <div className="min-h-screen bg-brand-navy flex items-center justify-center">
      <p className="text-white/60">Ladowanie...</p>
    </div>
  )
    ...
    <div className="min-h-screen bg-brand-navy">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex flex-wrap gap-2 mb-8">
          <button className={TAB_CLASS('services')} onClick={() => setTab('services')}>Uslugi</button>
        ...
        {tab === 'services' && (
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <p className="text-white/50 text-sm">Edytuj uslugi. Rozwiniety opis pojawia sie po kliknieciu "Dowiedz sie wiecej".</p>
              ...
              <div key={s.id} className="bg-brand-navyMid rounded-xl overflow-hidden">
                ...
                {openSection === s.id && (
                  <div className="px-6 pb-6 flex flex-col gap-4 border-t border-white/10 pt-4">
                    <div><label className={LABEL_CLASS}>Nazwa uslugi</label><input value={s.name} onChange={(e) => updateService(i, 'name', e.target.value)} className={INPUT_CLASS} /></div>
                    <div><label className={LABEL_CLASS}>Podtytul (po angielsku)</label><input value={s.subtitle} onChange={(e) => updateService(i, 'subtitle', e.target.value)} className={INPUT_CLASS} /></div>
                    <div><label className={LABEL_CLASS}>Krotki opis (na karcie)</label><textarea rows={2} value={s.description} onChange={(e) => updateService(i, 'description', e.target.value)} className={TEXTAREA_CLASS} /></div>
                    <div><label className={LABEL_CLASS}>Rozwiniety opis (po kliknieciu)</label><textarea rows={4} value={s.expanded} placeholder="Dodaj szczegolowy opis uslugi..." onChange={(e) => updateService(i, 'expanded', e.target.value)} className={TEXTAREA_CLASS + ' placeholder:text-white/30'} /></div>
                  </div>
                )}
              </div>
            ))}
        ...
        {tab === 'testimonials' && (
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <p className="text-white/50 text-sm">Zarzadzaj opiniami klientow.</p>
              <button onClick={() => setData({ ...data, testimonials: [...data.testimonials, { quote: '', author: '', location: '' }] })}
            ...
            {data.testimonials.map((t, i) => (
              <div key={i} className="bg-brand-navyMid rounded-xl p-6 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setData({ ...data, testimonials: data.testimonials.filter((_, idx) => idx !== i) })} className="text-red-400 hover:text-red-300 transition-colors"><Trash2 size={15} /></button>
                </div>
                <div><label className={LABEL_CLASS}>Tresc opinii</label><textarea rows={3} value={t.quote} onChange={(e) => { const u = [...data.testimonials]; u[i] = { ...t, quote: e.target.value }; setData({ ...data, testimonials: u }) }} className={TEXTAREA_CLASS} /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className={LABEL_CLASS}>Autor</label><input value={t.author} onChange={(e) => { const u = [...data.testimonials]; u[i] = { ...t, author: e.target.value }; setData({ ...data, testimonials: u }) }} className={INPUT_CLASS} /></div>
                  <div><label className={LABEL_CLASS}>Miasto</label><input value={t.location} onChange={(e) => { const u = [...data.testimonials]; u[i] = { ...t, location: e.target.value }; setData({ ...data, testimonials: u }) }} className={INPUT_CLASS} /></div>
                </div>
              </div>
            ))}
          </div>
        ...
        {tab === 'projects' && (
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <p className="text-white/50 text-sm">Zarzadzaj realizacjami.</p>
              <button onClick={() => setData({ ...data, projects: [...data.projects, { label: '', location: '', alt: '', image: '' }] })}
                className="flex items-center gap-2 text-brand-amber text-sm font-medium hover:text-brand-amberDark transition-colors">
                <Plus size={15} /> Dodaj realizacje
              </button>
            </div>
            {data.projects.map((p, i) => (
              <div key={i} className="bg-brand-navyMid rounded-xl p-6 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setData({ ...data, projects: data.projects.filter((_, idx) => idx !== i) })} className="text-red-400 hover:text-red-300 transition-colors"><Trash2 size={15} /></button>
                </div>
                <div>
                  <label className={LABEL_CLASS}>Zdjecie</label>
                  <div className="flex items-center gap-4">
                    {p.image ? (
                      <div className="relative w-24 h-16 rounded overflow-hidden flex-shrink-0"><Image src={p.image} alt={p.alt || 'Realizacja'} fill className="object-cover" /></div>
                    ) : (
                      <div className="w-24 h-16 rounded bg-brand-navy border border-white/10 flex items-center justify-center flex-shrink-0"><ImageIcon size={20} className="text-white/20" /></div>
                    )}
                    ...
                    <button onClick={() => fileInputRefs.current[i]?.click()} disabled={uploadingIndex === i}
                      ...
                      <Upload size={14} />
                      {uploadingIndex === i ? 'Wgrywanie...' : p.image ? 'Zmien zdjecie' : 'Dodaj zdjecie'}
                    </button>
                  </div>
                </div>
                <div><label className={LABEL_CLASS}>Nazwa realizacji</label><input value={p.label} onChange={(e) => { const u = [...data.projects]; u[i] = { ...p, label: e.target.value }; setData({ ...data, projects: u }) }} className={INPUT_CLASS} /></div>
                <div><label className={LABEL_CLASS}>Lokalizacja</label><input value={p.location} onChange={(e) => { const u = [...data.projects]; u[i] = { ...p, location: e.target.value }; setData({ ...data, projects: u }) }} className={INPUT_CLASS} /></div>
              </div>
            ))}
          </div>
        )}
        ...
        {tab === 'certificates' && (
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <p className="text-white/50 text-sm">Zarzadzaj certyfikatami. Kliknij "Wgraj zdjecie" aby dodac certyfikat.</p>
              ...
              {(data.certificates ?? []).map((cert, i) => (
                <div key={cert.id} className="bg-brand-navyMid rounded-xl p-4 flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => removeCertificate(i)} className="text-red-400 hover:text-red-300 transition-colors"><Trash2 size={13} /></button>
                  </div>
                  ...
                  <button onClick={() => certInputRefs.current[i]?.click()} disabled={uploadingCertIndex === i}
                    ...
                    <Upload size={12} />
                    {uploadingCertIndex === i ? 'Wgrywanie...' : cert.url ? 'Zmien' : 'Wgraj zdjecie'}
                  </button>
                </div>
              ))}
            </div>
